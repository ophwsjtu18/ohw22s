...
class House():
    def __init__(self,x,y,z):
        ...
        self.z  =  z
    def getPos(self):
        print("house located in (",self.x,",",self.y,",",self.z,")")
    ...
    def build(self,M,roofname):
        print("House will be built on (",self.x,",",self.y,",",self.z)
        print("House dimension(l,w,h) is (", self.l,",", self.w,",", self.h)
        f  =  open(roofname,"r")
        ...
        for i in range(self.l):
            for j in range(self.w):
                if roof[i][j]  == 0:
                    ...
                    mc.setBlock(self.x+i, self.y+self.h-1, self.z+j, block.GOLD_BLOCK.id)
